...
- (NSMutableArray *)systemProfileArrayForHost:(SUHost *)host
{
	...
	if (err == noErr)
		[profileArray addObject:[NSDictionary dictionaryWithObjects:@[@"cpuFreqMHz",@"CPU Speed (GHz)", @(gestaltInfo), @(gestaltInfo/1000.0)] forKeys:profileDictKeys]];
	
	// amount of RAM
	err = Gestalt(gestaltPhysicalRAMSizeInMegabytes,&gestaltInfo);
	if (err == noErr)
		[profileArray addObject:[NSDictionary dictionaryWithObjects:@[@"ramMB",@"Memory (MB)", @(gestaltInfo), @(gestaltInfo)] forKeys:profileDictKeys]];
